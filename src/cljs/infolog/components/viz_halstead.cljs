(ns infolog.components.viz-halstead
...
(defn join-data [max-difficulty mods data]
  (map (fn [[_ clauses]]
         (let [size (reduce + (map :effort clauses))
               difficulty (apply max (map :difficulty clauses))
               weight (double (/ difficulty max-difficulty))
               module-name (:module (first clauses))
               ptime (reduce + (map :time clauses))
               length (reduce + (map :length clauses))
               module-path (conj (vec (:path (mods module-name))) module-name)]
           {:module module-name
            :path module-path
            :name (:predicate (first clauses))
...
(defn mk-data [modules data]
  (let [mods (make-module-db @modules)
        max-difficulty (apply max (map :difficulty @data))
        joined (join-data max-difficulty mods @data)
    ...
    (.. points
        ...
        (on "mouseover" (fn [d]
                          (let [name (aget d "name")
                                name (if (= "root" name) "" name)
                                module (aget d "module")
                                difficulty (aget d "difficulty")
                                effort (aget d "effort")
                                ptime (aget d "ptime")
                                length (aget d "length")
                                x (aget d "x")
                                y (aget d "y")
                            ...
                            (reset! hover-text
                                    [(if module
                                      [:div
                                       ...
                                       [:div (str "Difficulty: " (format difficulty))]
                                       [:div (str "Effort: " (format effort))]
                                       [:div (str "Time: " (format ptime) " s")]
                                       [:div (str "Length: " length)]]
...
(defn viz []
  (let [modules (re-frame/subscribe [:raw-modules])
        data (mk-data modules (re-frame/subscribe [:halstead]))]
    (r/create-class
