(ns infolog.components.viz-dynamics
...
(defn join-data [max-depth mods data]
  (map (fn [item]
         (let [size (:dynamics-declared item)
               depth (:dynamics-declared item)
...
(defn mk-data [modules data]
  (let [mods (make-module-db @modules)
        max-depth (apply max (map :dynamics-declared @data))
...
(defn viz []
  (let [modules (re-frame/subscribe [:raw-modules])
       data (mk-data modules (re-frame/subscribe [:predstats]))]
    (r/create-class
